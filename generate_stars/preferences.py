...
def preferences_path() -> Path:
    return Path(GLib.get_user_config_dir()) / constants.PREFERENCES_DIR_NAME / constants.PREFERENCES_FILENAME
...
def load_last_save_path(path: Path | None = None) -> Path | None:
    ...
    raw_path = payload.get(constants.LAST_SAVE_PATH_KEY)
...
def save_last_save_path(last_save_path: Path, path: Path | None = None) -> None:
    ...
    payload = {constants.LAST_SAVE_PATH_KEY: str(last_save_path)}
